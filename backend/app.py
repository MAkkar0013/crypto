...
allowed_origins = [
    "https://crypto-frontend-tcnz.onrender.com/",
    "https://crypto-frontend-tcnz.onrender.com",
    "http://localhost:3000",
]
...
CORS(app, resources={
    ...
    r"/register": {"origins": "*", "methods": ["POST", "OPTIONS"]},
})
bcrypt = Bcrypt(app)

# Конфигурация
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', '').replace(
    'postgres://', 'postgresql://', 1  # Важно для Render PostgreSQL
) or 'sqlite:///local.db'
...
def after_request(response):
    # Дублируем настройки для надежности
    if request.referrer and any(origin in request.referrer for origin in allowed_origins):
...
def serve_frontend(path):
    # Для API запросов возвращаем 404
    ...
    # По умолчанию возвращаем index.html
    return send_from_directory(app.static_folder, 'index.html')

def home():
    return jsonify({"status": "API is running"}), 200  # Для health-чеков хоста
    return send_from_directory(app.static_folder, 'index.html')
    ...
    try:
        ...
        for item in portfolio_items:
            price_response = requests.get(
                f"https://crypto-backend-y9u6.onrender.com/api/coin-price/{item.coin_id}"
            )
