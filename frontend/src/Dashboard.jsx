...
const Dashboard = () => {
          ...
          response.data.map(async (coin) => {
            try {
              const priceResponse = await axios.get(
                `${API_URL}/api/coin-price/${coin.id}`
              );
              return {
                ...coin,
  ...
  const fetchCoinsData = async () => {
    try {
      ...
      const popularResponse = await axios.get(
        `${API_URL}/api/popular-cryptocurrencies`
      );
      ...
      if (Array.isArray(popularResponse.data)) {
        ...
        // Затем все монеты
        const allResponse = await axios.get(`${API_URL}/api/cryptocurrencies`);
  ...
  const getCurrentPrice = async (coinId) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/coin-price/${coinId}`);
  ...
  const addCoinToPortfolio = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/portfolio/${userId}`, {
  ...
  const removeCoinFromPortfolio = async (coinId) => {
    try {
      ...
      await axios.delete(`${API_URL}/api/portfolio/${userId}/${coinId}`);
  ...
  const fetchCoinDetails = async (coinId) => {
    try {
      ...
      const [detailsResponse, chartResponse] = await Promise.all([
        axios.get(`${API_URL}/api/coin/${coinId}`),
        axios.get(`${API_URL}/api/coin/${coinId}/market_chart?days=1`),
      ]);
