...
const Dashboard = () => {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      const response = await axios.get(`${API}api//portfolio/${userId}`);
  ...
  const fetchCoinsData = async () => {
    try {
      ...
      const popularResponse = await axios.get(
        `${API}api/popular-cryptocurrencies`
      );
      ...
      if (Array.isArray(popularResponse.data)) {
        ...
        // Затем все монеты
        const allResponse = await axios.get(`${API}api/cryptocurrencies`);
  ...
  const getCurrentPrice = async (coinId) => {
    try {
      ...
      const response = await axios.get(`${API}api/coin-price/${coinId}`);
  ...
  const addCoinToPortfolio = async () => {
    ...
    try {
      ...
      await axios.post(`${API}api/api/portfolio/${userId}`, {
  ...
  const removeCoinFromPortfolio = async (coinId) => {
    try {
      ...
      await axios.delete(`${API}api/portfolio/${userId}/${coinId}`);
  ...
  const fetchCoinDetails = async (coinId) => {
    try {
      ...
      const [detailsResponse, chartResponse] = await Promise.all([
        axios.get(`${API}api/api/coin/${coinId}`),
        axios.get(`${API}api/coin/${coinId}/market_chart?days=1`),
      ]);
